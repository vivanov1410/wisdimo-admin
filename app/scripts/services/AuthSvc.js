...
.factory('AuthSvc',
['$http', '$q', '$cookies', 'Globals',
function ($http, $q, $cookies, Globals) {
  ...
  Auth.login = function (credentials) {
    ...
    .success(function (account) {
      ...
      LocalStorageSvc.set( Globals.session_token_name, session_token );
  ...
  Auth.logout = function () {
    LocalStorageSvc.remove( Globals.session_token_name );
    delete this.account; 
  };

  Auth.set_session_token = function () {
    this._set_http_headers( LocalStorageSvc.get( Globals.session_token_name ) );
  };
  ...
  Auth.logged_in = function () {
    var session_token = LocalStorageSvc.get( Globals.session_token_name );
    if( session_token ) {
      return session_token.length >= Globals.min_session_token_length;
    }
